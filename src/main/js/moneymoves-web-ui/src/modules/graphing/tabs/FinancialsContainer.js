	...
	componentDidUpdate(prevProps) {
		if (this.props.fundamentals !== prevProps.fundamentals) {
			if (
				...
				this.props.fundamentals.instrumentFinancialsMap != null
			) {
				...
				if (firstKey != null) {
					const availableColumns = Object.keys(instrumentFinancialsMap[firstKey]);
					const columnFlags = [];
					...
					<CardHeader>
						<Row>
							<Col className="d-flex justify-content-end">
								<h5 style={{ margin: 0 }}>Financials Sheet for: {symbol}</h5>
							</Col>
							<Col className="d-flex justify-content-end">
							{/* Settings Button */}
